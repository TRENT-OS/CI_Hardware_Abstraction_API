...
class TTY_USB:
    ...
    def get_device_list(self_or_cls):
        ...
        for dev in sorted(os.listdir(base_folder)):
            ...
            def get_id_from_file(dn, id_file):
                ...
                if not os.path.exists(id_file_fqn):
                    return None
            ...
            driver = os.path.basename(
                os.path.realpath(os.path.join(dev_fqn, "device/driver"))
            )

            device = TTY_USB(f"/dev/{dev}", vid, pid, serial, usb_path, driver)
    ...
    def get_and_print_device_list(self_or_cls):
        ...
        for dev in dev_list:
            ...
            print(
                f"  {dev.device} is {dev.vid}:{dev.pid} {sn} at {dev.usb_path}, driver {dev.driver}"
            )
    ...
    def find_device(self_or_cls, serial=None, usb_path=None):
        ...
        if usb_path and (usb_path != my_device.usb_path):
            raise Exception(
                f"USB path different, expected {usb_path}, got {my_device.usb_path}"
            )

        if serial and (serial != my_device.serial):
            raise Exception(
                f"serial different, expected {serial}, got {my_device.serial}"
            )
